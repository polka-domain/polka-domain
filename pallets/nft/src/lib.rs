...
pub mod module {
	...
	pub(super) type ReservedNFT<T: Config> = StorageMap<
		_,
		Blake2_128Concat,
...
impl<T: Config> Pallet<T> {
	...
	fn do_transfer(
		...
		ensure!(!<ReservedNFT<T>>::contains_key(token), Error::<T>::NoPermission);
	...
	fn do_reserve(who: &T::AccountId, token: (ClassIdOf<T>, TokenIdOf<T>)) -> DispatchResult {
		...
		<ReservedNFT<T>>::insert(token, who);
	...
	fn do_unreserve(who: &T::AccountId, token: (ClassIdOf<T>, TokenIdOf<T>)) -> DispatchResult {
		...
		ensure!(<ReservedNFT<T>>::get(token) == Some(who.clone()), Error::<T>::NoPermission);
		<ReservedNFT<T>>::remove(token);
...
impl<T: Config> Mutate<T::AccountId> for Pallet<T> {
	...
	fn mint_into(
		...
		let class_owner = <Self as Inspect<T::AccountId>>::class_owner(class)
	...
	fn burn(
		...
		let owner = <Self as Inspect<T::AccountId>>::owner(class, instance)
...
impl<T: Config> ReserveNFT<T::AccountId> for Pallet<T> {
	type ClassId = ClassIdOf<T>;

...
use super::*;
pub use crate::mock::{
	AuctionModule, Event, ExtBuilder, NFTPallet, Origin, Proxy, System, Tokens, ALICE, BOB,
	CLASS_ID, TOKEN_ID,
};
...
fn test_bid_auction_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			AuctionModule::bid_auction(Origin::signed(BOB), 0, 1),
		...
		assert_noop!(
			AuctionModule::bid_auction(Origin::signed(BOB), 0, 2),
			Error::<Runtime>::InvalidBidAmount
		);
		...
		assert_noop!(
			AuctionModule::bid_auction(Origin::signed(BOB), 0, 3),
...
fn test_cancel_auction_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			AuctionModule::cancel_auction(Origin::signed(BOB), 0),
		...
		assert_noop!(
			AuctionModule::cancel_auction(Origin::signed(ALICE), 0),
